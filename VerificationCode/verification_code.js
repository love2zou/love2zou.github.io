...
function buttonCountdown($el, msNum, timeFormat) {
    var text = $el.text(),//返回被选元素的文本内容
    	timer = 0;
    $el.prop("disabled", true).addClass("disabled")//修改disabled属性，添加disabled属性让其显示
...
$("#test2-clear").on("click",function(){
    $("#test2").trigger("timer.clear");//触发被选元素的指定事件类型:timer.clear
});
...
$("#test3-clear").on("click",function(){
    $("#test3").trigger("timer.clear");//触发被选元素的指定事件类型:timer.clear
});
...
$("#test4-clear").on("click",function(){
    $("#test4").trigger("timer.clear");//触发被选元素的指定事件类型:timer.clear
});